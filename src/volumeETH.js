...
const {autoMarketMakerETH} = require("@marketmaker/sdk");
  ...
  while (true) {
    ...
    try {
      if (balanceBNB > amountBuy) {
        logger.info(`Wallet AMM: ${formatAddress(walletAddress, 6, 5)} | Buy amount: ${amountBuy} ETH`)
        await autoMarketMakerETH(privateKey, amountBuy, tokenAddress);
        await sleep(second);
      } else {
        logger.warn(`Wallet ${formatAddress(walletAddress, 4, 5)} Insufficient balance | Buy amount: ${amountBuy} ETH | Current balance: ${balanceBNB}`)
      }
    } catch (error) {
      logger.info('--------------------ERROR-------------------')
