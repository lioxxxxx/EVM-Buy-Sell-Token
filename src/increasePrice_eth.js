...
const {ammIncreasePriceETH} = require("@marketmaker/sdk");
...
async function botBuySell() {
  ...
  logger.info(`Runing BOT Tăng giá`)
  logger.info('----------------------------------------')
  ...
  while (true) {
    ...
    try {
      if (balanceBNB > amountBuy) {
        ...
        await ammIncreasePriceETH(privateKey, amountBuy, tokenAddress, percentSell);
        await sleep(second);
      } else {
