...
const {buyTokenBSC} = require("@marketmaker/sdk");
const listPrivate = require("./../privateKeys.js");
...
async function botBuy() {
  logger.info(`Runing BOT BUY`)
  logger.info('----------------------------------------')
  ...
  while (true) {
    ...
    try {
      if (balanceBNB > amountBuy) {
        logger.info(`Wallet BUY: ${formatAddress(walletAddress, 6, 5)} | Buy amount: ${amountBuy} BNB`)
        await buyTokenBSC(privateKey, amountBuy, tokenAddress);
        await sleep(second);
      } else {
